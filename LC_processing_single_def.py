...
file.write(s)
m = m1|m4|m5
s = 'There are '+str(np.sum(m)) + ' points out of ' + str(len(m)) + ' that fulfill sigma OR  chi2R  OR chi2DOF  \n '
file.write(s)
m = m1*(m4|m5)
s = 'There are '+str(np.sum(m)) + ' points out of ' + str(len(m)) + ' that fulfill sigma  AND  (chi2R OR chi2DOF)  \n '
file.write(s)
file.write('\n')
file.write('Output :  \n')
s = '    '+ DirOut + '\n' 
